    ...
    if raalgo == 'CSMA':
        ...
        while currentslot < beaconinterval/slottime - frametxslot:
            ...
            # Tx succ
                ...
                df2 = pd.DataFrame({'time': currentslot*slottime, 'node': ind + 1, 'aoi': aoitimestamp[ind]*slottime, 'result': 'succ'})
            ...
            # Tx failed
                ...
                df2 = pd.DataFrame({'time': currentslot*slottime, 'node': ind + 1, 'aoi': aoitimestamp[ind]*slottime, 'result': 'fail'})
                df = pd.concat([df, df2], ignore_index=True, axis=0)
                aoitimestamp[retx == 0] = currentslot
                num_fail += 1
            # Tx coll
                ...
                for x in ind:
                    ...
                    bo_counter[x] = np.random.randint(contentionwindowsize[bo_stage[x]])
                # print(f"Time: {currentslot}, Tx collision from {ind+1} with AoI {aoi[ind]}")
                df2 = pd.DataFrame({'time': currentslot*slottime, 'node': ind + 1, 'aoi': aoitimestamp[ind]*slottime, 'result': 'coll'})
                df = pd.concat([df, df2], ignore_index=True, axis=0)
                aoitimestamp[retx == 0] = currentslot
                num_coll += 1
